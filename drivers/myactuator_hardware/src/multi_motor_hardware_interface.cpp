  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  // 2. 홈 위치에서 위치 유지 (stopMotor 호출 안 함)
  //    stopMotor()를 호출하면 토크가 사라져서 로봇이 중력에 의해 떨어짐
  //    대신 홈 위치(0)를 계속 유지하도록 마지막 명령 전송
  RCLCPP_INFO(getLogger(), "[2/2] Holding position at home (not stopping motors)...");
  
  for (auto & motor : motors_) {
    if (motor.actuator) {
      try {
        // 홈 위치(0)를 유지하도록 마지막 위치 명령 전송
        motor.actuator->sendPositionAbsoluteSetpoint(0.0f, 10.0f);
        RCLCPP_INFO(getLogger(), "  → '%s': holding at home", motor.joint_name.c_str());
      } catch (const std::exception & e) {
        RCLCPP_WARN(getLogger(), "  → '%s': hold command failed (%s)", 
          motor.joint_name.c_str(), e.what());
      }
    }
  }
  ...
  RCLCPP_INFO(getLogger(), "  ⚠ Motors will lose torque when program exits!");
  RCLCPP_INFO(getLogger(), "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
  RCLCPP_INFO(getLogger(), " ");
